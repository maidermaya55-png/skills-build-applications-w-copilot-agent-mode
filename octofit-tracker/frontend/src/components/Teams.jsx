...
function Teams() {
  ...
  useEffect(() => { fetch(`${API_BASE_URL}/api/teams/`).then((response) => { if (!response.ok) throw new Error(); return response.json() }).then((payload) => { setTeams(getItems(payload)); setStatus('ready') }).catch(() => setStatus('error')) }, [])
