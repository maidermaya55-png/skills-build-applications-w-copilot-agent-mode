...
function Workouts() {
  ...
  useEffect(() => { fetch(`${API_BASE_URL}/api/workouts/`).then((response) => { if (!response.ok) throw new Error(); return response.json() }).then((payload) => { setWorkouts(getItems(payload)); setStatus('ready') }).catch(() => setStatus('error')) }, [])
