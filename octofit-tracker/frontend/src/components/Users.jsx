...
function Users() {
  ...
  useEffect(() => { fetch(`${API_BASE_URL}/api/users/`).then((response) => { if (!response.ok) throw new Error(); return response.json() }).then((payload) => { setUsers(getItems(payload)); setStatus('ready') }).catch(() => setStatus('error')) }, [])
