...
const LEADERBOARD_ENDPOINT = codespaceName ? `https://${codespaceName}-8000.app.github.dev/api/leaderboard/` : `${API_BASE_URL}/api/leaderboard/`
const getItems = (payload) => Array.isArray(payload) ? payload : payload?.results || payload?.data || payload?.items || []

function Leaderboard() {
  const [leaders, setLeaders] = useState([])
  const [status, setStatus] = useState('loading')
  useEffect(() => { fetch(LEADERBOARD_ENDPOINT).then((response) => { if (!response.ok) throw new Error(); return response.json() }).then((payload) => { setLeaders(getItems(payload)); setStatus('ready') }).catch(() => setStatus('error')) }, [])
